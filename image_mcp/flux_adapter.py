...
class FluxAdapter:
    ...
    async def generate(self, prompt: DiffusionPrompt) -> Tuple[str, Dict[str, Any]]:
    ...
    def _poll_for_result(self, polling_url: str, request_id: str, max_wait: int) -> Dict[str, Any]:
        ...
        while time.time() - start < max_wait:
            time.sleep(0.5)
            try:
                r = self._session.get(polling_url, params={"id": request_id}, timeout=(self.connect_timeout, self.read_timeout))
                r.raise_for_status()
                result = r.json()
